...
class SequenceGraphBuilder:
    ...
    def parse_metadata(self, text):
        ...
        # Extract headers (assumed tab-separated)
    ...
    def create_labeled_base_node(self, session, pos, sym):
        cypher = "MERGE (n:Mutation:Base {position: $pos, symbol: $sym})"
        session.run(cypher, pos=pos, sym=sym)

    def create_nodes_and_relationships(self, sequences, metadata=None):
        ref_id = "ref"
        ref_seq = sequences[ref_id]
        length = len(ref_seq)
        # Pad all sequences to match the length of the reference
        for k in sequences:
        ...
        with self.driver.session() as session:
            ...
            for i, symbol in enumerate(ref_seq):
                if symbol == '-':
                    ...
                    if i == 0:
                        session.run(
                            "CREATE (:Head:Base {seq_id: $seq_id, position: $pos, symbol: $sym})",
                            seq_id=ref_id, pos=i, sym=symbol
                        )
                        ref_nodes.append((ref_id, i))
                    elif i == length-1:
                        session.run(
                            "CREATE (:Tail:Base {seq_id: $seq_id, position: $pos, symbol: $sym})",
                            seq_id=ref_id, pos=i, sym=symbol
                        )
                        ref_nodes.append((ref_id, i))
                    else:
                        session.run(
                            "CREATE (:Base {seq_id: $seq_id, position: $pos, symbol: $sym})",
                            seq_id=ref_id, pos=i, sym=symbol
                        )
                        ref_nodes.append((ref_id, i))

            # Create NEXT relationships for reference
            # Create NEXT relationships for reference (only once)
            for i in range(length - 1):
                ...
                session.run(
                    ...
                    CREATE (a)-[:NEXT_REF]->(b)
                    """,
                    from_id=from_id, from_pos=from_pos,
            ...
            for seq_id, seq in sequences.items():
                ...
                for i in range(length):
                    base = seq[i]
                    if base == ref_seq[i]:
                        node_ids.append((ref_id, i))  # Use ref node
                    else:
                        if base == '-':
                            node_ids.append(None)
                        else:
                            # Create new node
                            self.create_labeled_base_node(session, i, base)
                            node_ids.append((None, i))
                all_node_ids[seq_id] = node_ids

                # Create NEXT_SEQ edges skipping gaps
                for i in range(length - 1):
                    from_sym = seq[i]
                    j = i + 1
                    ...
                    from_id, from_pos = from_node
                    to_id, to_pos = to_node
                    ...
                    session.run(
                        ...
                        to_sym=seq[to_pos], to_pos=to_pos,
                        seq_id=seq_id
                    )

            #create the pointer nodes for each sequence
            for seq_id, seq in sequences.items():
                if metadata:
                    ...
                    query = f"CREATE (:Pointer {{{props}}})"

                    session.run(query, **params)
                else:
                    session.run(
                        "CREATE (:Pointer {seq_id: $seq_id})",
                        seq_id=seq_id
                    )
...
if __name__ == "__main__":
    # Replace with your own Neo4j credentials
    ...
    builder = SequenceGraphBuilder(URI, USER, PASSWORD)
